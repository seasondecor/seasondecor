...
const QuotationDocument = ({ data = {} }) => {
  ...
  const safetyProducts = Array.isArray(relatedProductItems)
    ? relatedProductItems
    : [];
  ...
  const materialTotal = safetyMaterials.reduce((sum, item) => {
    ...
    console.log(
      `Material ${item.materialName}: raw cost=${
        item.cost
      }, parsed cost=${cost}, qty=${quantity}, total=${cost * quantity}`
    );
  ...
  const constructionTotal = safetyTasks.reduce((sum, item) => {
    ...
    console.log(
      `Task ${item.taskName}: raw cost=${
        item.cost
      }, parsed cost=${cost}, area=${area}, total=${cost * area}`
    );
  ...
  const productsTotal = safetyProducts.reduce((sum, product) => {
    const unitPrice = parseFormattedNumber(product.unitPrice);
    const quantity = parseFormattedNumber(product.quantity);
    console.log(
      `Product ${product.productName}: raw price=${
        product.unitPrice
      }, parsed price=${unitPrice}, qty=${quantity}, total=${
        unitPrice * quantity
      }`
    );
  ...
  const grandTotal = materialTotal + constructionTotal + productsTotal;

  // Calculate final total after deducting commitment deposit
  const finalTotal = grandTotal - 500000;
      ...
      <Page size="A3" style={styles.page}>
        ...
        <View style={styles.infoBox}>
          <View
            style={[
              styles.infoColumn,
              { backgroundColor: "#f9f9f9", flex: 1.2 },
            ]}
          >
        ...
        <View style={[styles.section, { marginBottom: 8 }]}>
          <Text
            style={[styles.sectionTitle, { color: "#333", marginBottom: 5 }]}
          >
              ...
              <View>
                {Array.isArray(scopeOfWorks) && scopeOfWorks.length > 0 ? (
                  scopeOfWorks.map((work, index) => (
                    <Text
                      key={`work-${index}`}
                      style={[styles.infoValue, { fontSize: 8 }]}
                    >
        ...
        {safetyProducts.length > 0 && (
          <View style={[styles.section, { marginBottom: 8 }]}>
            <Text
              style={[
                styles.sectionTitle,
                { color: "#FF385C", marginBottom: 5 },
              ]}
            >
              Products Add-Ons
            </Text>
            <View style={[styles.table, styles.compactRows]}>
              ...
              <View style={[styles.tableRow, styles.tableHeader]}>
                <View style={[styles.tableCell, { flex: 3 }]}>
                  <Text
                    style={{ color: "white", fontWeight: "bold", fontSize: 7 }}
                  >
                    Product Name
                  </Text>
                </View>
                ...
                return (
                  <View
                    ...
                    style={[
                      styles.tableRow,
                      {
                        backgroundColor: isEvenRow ? "#fff" : "#f8f8f8",
                        minHeight: 20,
                      },
                    ]}
                  >
                    <View style={[styles.tableCell, { flex: 3 }]}>
                      <Text style={{ fontSize: 7 }}>{product.productName}</Text>
                    </View>
                    <View style={[styles.tableCell, { flex: 0.8 }]}>
                      <Text style={{ fontSize: 7 }}>{quantity}</Text>
                    </View>
                    <View style={[styles.tableCell, { flex: 1.2 }]}>
                    ...
                    <View style={[styles.tableCell, { flex: 1.2 }]}>
                      <Text style={{ fontSize: 7 }}>{formatVND(total)}</Text>
                    </View>
                  </View>
                );
              })}
            ...
            {/* Products Total */}
            <View style={[styles.subtotal, { marginTop: 4 }]}>
              <Text style={styles.totalLabel}>Products Total:</Text>
              <Text style={styles.totalValue}>{formatVND(productsTotal)}</Text>
            </View>
          </View>
        )}
        ...
        <View style={styles.section}>
          <Text
            style={[styles.sectionTitle, { color: "#FF385C", marginBottom: 5 }]}
          >
            Quotation Breakdown
          </Text>
                ...
                safetyMaterials.slice(0, 3).map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={[styles.noteCell, { flex: 0.8 }]}>
                        <Text style={{ fontSize: 7 }}>
                          {item.note || "N/A"}
                        </Text>
                      </View>
          ...
          <View style={[styles.tableContainer, { marginTop: 5 }]}>
            ...
            <View style={styles.subtotal}>
              ...
              <Text style={styles.totalValue}>
                {formatVND(constructionTotal)}
              </Text>
            </View>
          </View>
        </View>

        {/* Price Summary Section - Vertical Stack */}
        <View style={[styles.section, { marginTop: 10, padding: 0, borderLeft: 'none' }]}>
          <View style={{ gap: 8 }}>
            <View style={[styles.total, { backgroundColor: '#333', height: 36, justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row', paddingHorizontal: 16 }]}>
              <Text style={[styles.totalLabel, { color: 'white', fontSize: 10 }]}>Subtotal:</Text>
              <Text style={[styles.totalValue, { color: 'white', fontSize: 10 }]}>{formatVND(grandTotal)}</Text>
            </View>

            <View style={[styles.total, { backgroundColor: '#FF385C', height: 36, justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row', paddingHorizontal: 16 }]}>
              <Text style={[styles.totalLabel, { color: 'white', fontSize: 10 }]}>Commitment Deposit:</Text>
              <Text style={[styles.totalValue, { color: 'white', fontSize: 10 }]}>- {formatVND(500000)}</Text>
            </View>

            <View style={[styles.total, { backgroundColor: '#000', height: 36, justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row', paddingHorizontal: 16 }]}>
              <Text style={[styles.totalLabel, { color: 'white', fontSize: 10 }]}>Final Total:</Text>
              <Text style={[styles.totalValue, { color: 'white', fontSize: 10 }]}>{formatVND(finalTotal)}</Text>
            </View>

            <View style={[styles.total, { backgroundColor: '#666', height: 36, justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row', paddingHorizontal: 16 }]}>
              <Text style={[styles.totalLabel, { color: 'white', fontSize: 10 }]}>Deposit Amount ({depositPercent}%):</Text>
              <Text style={[styles.totalValue, { color: 'white', fontSize: 10 }]}>{formatVND(depositAmount)}</Text>
            </View>
            
            <View style={{ paddingHorizontal: 16, paddingTop: 4 }}>
              <Text style={{ fontSize: 8, color: '#666', fontStyle: 'italic' }}>
                * The deposit amount will be charged when you start depositing the contract
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={[styles.section, { marginBottom: 0 }]}>
          <Text style={[styles.sectionTitle, { color: "#333", fontSize: 12 }]}>
            Terms and Conditions
          </Text>
          <Text style={[styles.termsText, { fontSize: 7 }]}>
            {terms ||
              "Payment due within 30 days. Cancellation policy: 50% refund if canceled 30 days before the event."}
          </Text>
        </View>

        {/* Footer */}
        <View style={[styles.footer, { paddingTop: 5 }]}>
          <Text style={[styles.footerText, styles.highlight, { fontSize: 7 }]}>
            Season Decor - A Decoration Platform
          </Text>
          <Text style={[styles.footerText, { fontSize: 6 }]}>
            www.seasondecor.com | info@seasondecor.com | +84 123 456 789
          </Text>
        </View>
      </Page>
    </Document>
...
export const generatePdfBlob = async (quotationData) => {
  ...
  const enhancedData = {
    ...
    // These fields are already flattened in the quotationData
    spaceType: quotationData.spaceType || "Not specified",
    roomSize: quotationData.roomSize || "Not specified",
    primaryUser: quotationData.primaryUser || "Not specified",
