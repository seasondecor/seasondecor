...
import { useMotionValue, motion, useMotionTemplate, animate, useAnimate } from "motion/react";
...
export const HeroHighlight = ({
  ...
  useEffect(() => {
    const autoAnimate = async () => {
      while (true) {
        ...
        await animate(mouseX, window.innerWidth, {
          duration: 3,
          ease: "easeInOut",
        });
        // Move from right to left
        await animate(mouseX, 0, {
          duration: 3,
          ease: "easeInOut",
        });
      }
    ...
    const autoAnimateY = async () => {
      while (true) {
        // Move from top to bottom
        await animate(mouseY, window.innerHeight, {
          duration: 4,
          ease: "easeInOut",
        });
        // Move from bottom to top
        await animate(mouseY, 0, {
          duration: 4,
          ease: "easeInOut",
        });
      }
  ...
  return (
    <div
      ref={scope}
      className={cn(
        "group relative flex h-full w-full items-center justify-center bg-white dark:bg-black z-10",
        containerClassName
      )}
      onMouseMove={handleMouseMove}>
       <div className="absolute inset-x-0 bottom-0 z-10 h-[900px] bg-gradient-to-t from-white to-transparent dark:from-black opacity-90 rounded-2xl"></div>
      <div
